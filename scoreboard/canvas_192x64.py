...
class Canvas:
    ...
    def show_timer(self, msg, count):
        ...
        if msg:
            ...
            graphics.DrawText(self.canvas, self.mesg_font, x, 20, self.mesg_color, msg.center(l))

            msg2 = f'{int(count/60):2}:{(int(count)%60):02}'
            [x,l] = [10,11] if (len(msg2) % 2 == 0) else [2,12]
            graphics.DrawText(self.canvas, self.time_font, x, 50, self.score_color, msg2.center(l))
            self.canvas = self.matrix.SwapOnVSync(self.canvas)
        else:
            msg2 = f'{int(count/60):2}:{(int(count)%60):02}'
            [x,l] = [10,11] if (len(msg2) % 2 == 0) else [2,12]
            graphics.DrawText(self.canvas, self.time_font, x, 40, self.score_color, msg2.center(l))
            self.canvas = self.matrix.SwapOnVSync(self.canvas)
